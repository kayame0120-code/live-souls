...
公演タブと同様に、当落タブもグループ→ツアー→申込詳細の3階層構造に変更:
- 第1層: 自分の申込があるグループのカード一覧（当落待ちあり表示）
- 第2層: グループ内ツアー一覧（当落待ちあり/発表済）
- 第3層: 従来の申込詳細(showByTour)

### 公演ツアー一覧の編集ボタントグル化

通常時は移動セレクト/削除ボタンを非表示にし、右上「編集」ボタンクリック時のみ表示。
もう一度クリックで「完了」→非表示に戻る。
