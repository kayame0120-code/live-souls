...
### QV27-1: リカバリーキー紛失警告UIの完全実装

- **基準根拠**: security_criteria No.7「リカバリーキーを提示できない場合の復旧不能がUI上で明示されているか」
- **状況**: サーバー側基盤(e2e_keysテーブル・API)とクライアント側暗号モジュール(e2e-crypto.js)は完成。リカバリーキー初回提示UIの画面実装(Blade + JS統合)が未完。
- **仮置き(安全側)**: 基盤は動作するが、初回セットアップ時のリカバリーキー提示フロー(モーダル/専用画面)のUI仕上げを次段に送る。

### QV27-2: E2E暗号化のクライアント側UI統合

- **基準根拠**: security_criteria No.11「暗号化はクライアント側で完了しているか」
- **状況**: e2e-crypto.jsにencrypt/decrypt/setupE2E等の全関数を実装済み。名義登録/編集/複製フォームからe2e-crypto.jsを呼び出してフォーム送信前に暗号化する統合(form onsubmitフック)が未完。
- **仮置き(安全側)**: 統合前の現状は従来通りの動作(member_no/login_id/passwordは平文でサーバーに届くがencryptedキャスト除去済みのため暗号化されずに保存される)。**本番デプロイ前にこの統合を完了する必要がある**。
